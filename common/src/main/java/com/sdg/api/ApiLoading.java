...
public class ApiLoading {
    ...
    public void show(final String message, final boolean isCancel) {
        ...
        cancel();
        mDialog = new ProgressDialog(RetrofitManager.Companion.get().getContext(),ProgressDialog.STYLE_HORIZONTAL);
        mDialog.setCanceledOnTouchOutside(false);
        mDialog.setCancelable(isCancel);
        mDialog.show();
        WindowManager.LayoutParams params = mDialog.getWindow().getAttributes();
        ...
        params.height = px2dp(130);
        mDialog.getWindow().setAttributes(params);
        View v = LayoutInflater.from(RetrofitManager.Companion.get().getContext()).inflate(R.layout.api_loading, null);
        ...
        ImageView iv_loading = v.findViewById(R.id.loading);
        mDialog.setContentView(v);
        initAnimation(RetrofitManager.Companion.get().getContext(),iv_loading);
    }
    /**
     * 初始化默认的动画
     * */
    ...
    public static void post(Runnable task) {
        ...
        //判断当前线程是否是主线程
        if (RetrofitManager.Companion.get().getMainThreadId() == android.os.Process.myTid()) {
            //是主线程
            task.run();
        } else {
            //不是主线程
            RetrofitManager.Companion.get().getHandler().post(task);
        }
    }

    public static int px2dp(int px){
        return  RetrofitManager.Companion.get().getContext().getResources().getDimensionPixelSize(R.dimen.base_dp)*px;
    }
}
